...
Bei der schubstarren Platte nach Kirchhoff wird die Verformung durch $w$ beschrieben. $\theta_x$ und $\theta_y$ werden durch die Ableitung der Verformung beschrieben. Für die Näherungslösung der Verformung werden Basisfunktionen $\varphi_1, \varphi_2,...,\varphi_N$ gewählt, sodass $w_h$ durch die Funktion
$$
\begin{align}
...
wobei $V_h$ ein Unterraum von $V$ ist und $N$ die Dimension des Raums $V_h$. $\varphi_i$ sind die Elemente des Vektorraums $V_h$. Das sich daraus ergebende Problem wird _abstracktes, diskretes Variationsproblem_ bezeichnet.
...
$$
\begin{align}
a(w_h, \sum_{i = 1}^N \varphi_i \cdot \delta \hat{w}_i)= b(\sum_{i = 1}^N \varphi_i \cdot \delta \hat{w}_i) \\
...
$$
a(\sum_{j = 1}^N \varphi_j \cdot \hat{w}_j, \varphi_i)= b(\varphi_i)
$$

$$
\sum_{j = 1}^N \underbrace{a(\varphi_j, \varphi_i)}_{k_{ij}} \cdot \hat{w}_j= \underbrace{b(\varphi_i)}_{r_j}
$$


----
Zur Herleitung des linearen Gleichungssystems für das gesamte System, aufgeteilt in mehrere Plattenelemente, wird zunächst die Bilinearform (@eq-bilinearform) für jedes einzelne Element ausgewertet, wodurch sich die jeweiligen Elementsteifigkeitsmatrizen ergeben. Durch die Assemblierung der Elementsteifigkeitsmatrizen lässt sich die globale Steifigkeitmatrix ableiten.
