...
Eine Funktion wird $C^n$-Funktion genannt, wenn die Ableitung vom Grad $j$ mit $0 \le j \le n$ stetig in allen Punkten ist. Bei der FEM sind vorrangig $C^{-1}$-, $C^{0}$- und $C^{1}$-Funktionen von bedeutung. Beispiele dieser sind in ... dargestellt. Die Ableitung einer C^0 Funktionen ist stetig, außer in den Punkten an denen die Funktion einen Knick hat. Dementsprechend ist einen die Ableitung der $C^0$-Funktioen eine $C^{-1}$-Funktion welche an den Knicken einen Sprung aufweist. Generell ist die Ableitungen eine $C^n$ Funktion eine $C^{n-1}$ Funktion.
...
Die Formfunktionen der oben beschriebenen, unterschiedlichen finite Elemente Ansätze, werden elementweise definiert. In Abbildung ... sind zufällige Kombinationen der Formfunktionen des

a) bilinearen Elements

b) konformen Hermite Elements

c) nichtkonformen Hermite Elements

d) Elements der Serendipity Klasse
...
TODO: Abbildungen erstellen + beschreiben!
