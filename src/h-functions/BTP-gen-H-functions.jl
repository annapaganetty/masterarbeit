...
function btpHx(e)
    ...
    Hx[3] = simplify(sum([0,0,0,0,(-3S₅) / (2l₁₂),0,0,(3S₈) / (2l₄₁)] .* Ns))
    Hx[2] = simplify(sum([0,0,0,0,-0.75C₅*S₅,0,0,-0.75C₈*S₈] .* Ns))
    Hx[1] = simplify(sum([1,0,0,0,(1//16)*(8(C₅^2) - 4(S₅^2)),0,0,(1//16)*(8(C₈^2) - 4(S₈^2))] .* Ns))
    Hx[6] = simplify(sum([0,0,0,0,(3S₅) / (2l₁₂),(-3S₆) / (2l₂₃),0,0] .* Ns))
    Hx[5] = simplify(sum([0,0,0,0,-0.75C₅*S₅,-0.75C₆*S₆,0,0] .* Ns))
    Hx[4] = simplify(sum([0,1,0,0,(1//16)*(8(C₅^2) - 4(S₅^2)),(1//16)*(8(C₆^2) - 4(S₆^2)),0,0] .* Ns))
    Hx[9] = simplify(sum([0,0,0,0,0,(3S₆) / (2l₂₃),(-3S₇) / (2l₃₄),0] .* Ns))
    Hx[8] = simplify(sum([0,0,0,0,0,-0.75C₆*S₆,-0.75C₇*S₇,0] .* Ns))
    Hx[7] = simplify(sum([0,0,1,0,0,(1//16)*(8(C₆^2) - 4(S₆^2)),(1//16)*(8(C₇^2) - 4(S₇^2)),0] .* Ns))
    Hx[12] = simplify(sum([0,0,0,0,0,0,(3S₇) / (2l₃₄),(-3S₈) / (2l₄₁)] .* Ns))
    Hx[11] = simplify(sum([0,0,0,0,0,0,-0.75C₇*S₇,-0.75C₈*S₈] .* Ns))
    Hx[10] = simplify(sum([0,0,0,1,0,0,(1//16)*(8(C₇^2) - 4(S₇^2)),(1//16)*(8(C₈^2) - 4(S₈^2))] .* Ns))
    return Hx
...
function btpHy(e)
    ...
    Hy[2] = simplify(sum([0,0,0,0,(3C₅) / (2l₁₂),0,0,(-3C₈) / (2l₄₁)] .* Ns))
    Hy[1] = simplify(sum([-1,0,0,0,(1//16)*(4(C₅^2) - 8(S₅^2)),0,0,(1//16)*(4(C₈^2) - 8(S₈^2))] .* Ns))
    Hy[3] = simplify(sum([0,0,0,0,0.75C₅*S₅,0,0,0.75C₈*S₈] .* Ns))
    Hy[5] = simplify(sum([0,0,0,0,(-3C₅) / (2l₁₂),(3C₆) / (2l₂₃),0,0] .* Ns))
    Hy[4] = simplify(sum([0,-1,0,0,(1//16)*(4(C₅^2) - 8(S₅^2)),(1//16)*(4(C₆^2) - 8(S₆^2)),0,0] .* Ns))
    Hy[6] = simplify(sum([0,0,0,0,0.75C₅*S₅,0.75C₆*S₆,0,0] .* Ns))
    Hy[8] = simplify(sum([0,0,0,0,0,(-3C₆) / (2l₂₃),(3C₇) / (2l₃₄),0] .* Ns))
    Hy[7] = simplify(sum([0,0,-1,0,0,(1//16)*(4(C₆^2) - 8(S₆^2)),(1//16)*(4(C₇^2) - 8(S₇^2)),0] .* Ns))
    Hy[9] = simplify(sum([0,0,0,0,0,0.75C₆*S₆,0.75C₇*S₇,0] .* Ns))
    Hy[11] = simplify(sum([0,0,0,0,0,0,(-3C₇) / (2l₃₄),(3C₈) / (2l₄₁)] .* Ns))
    Hy[10] = simplify(sum([0,0,0,-1,0,0,(1//16)*(4(C₇^2) - 8(S₇^2)),(1//16)*(4(C₈^2) - 8(S₈^2))] .* Ns))
