...
$$
\begin{align}
  C_{5} &= cos(\gamma_{5}) = cos(270°) = 0 \\
  C_{6} &= cos(\gamma_{6}) = cos(0°) = 1\\
  C_{7} &= cos(\gamma_{7}) = cos(90°) = 0 \\
  C_{8} &= cos(\gamma_{8}) = cos(180°) = -1 \\
\end{align}
$$
$$
\begin{align}
  S_{5} &= sin(\gamma_{5}) = sin(270°) = -1 \\
  S_{6} &= sin(\gamma_{6}) = sin(0°) = 0 \\
  S_{7} &= sin(\gamma_{7}) = sin(90°) = 1 \\
  S_{8} &= sin(\gamma_{8}) = sin(180°) = 0 \\
\end{align}
$$
...
$$
\begin{align}
  -w_{,s_{5}} &= (\frac{3}{2 l_{12}} (w_{1} - w_{2}) + \frac{1}{4} (w_{,s_{1}} + w_{,s_{2}})) \\
  ...
  \beta_{n_{6}}   &= \frac{1}{2} (\beta_{n_{2}} + \beta_{n_{3}}) 
  ...
  w_{,s_{7}}  &= -(\frac{3}{2 l_{34}} (w_{3} - w_{4}) + \frac{1}{4} (w_{,s_{3}} + w_{,s_{4}})) \\
  ...
  -\beta_{n_{8}}  &= -\frac{1}{2} (\beta_{n_{4}} + \beta_{n_{1}}) 
...
$\beta_x$ kann auch in der folgenden Matrixschreibweise ausgedrückt werden: 
...
Werte für ein quadratisches 2x2 Element:
...
\begin{align}
    ...
    = \left( \begin{array}{rrr} N_4 & N_8 & N_7 \\ \end{array}\right) \cdot \left( \begin{array}{rrr} 1 \\ 0,5 \\ 0,25 \\ \end{array}\right)w \\
