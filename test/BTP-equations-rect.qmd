...
\
\
Um die Ableitung der Funktionen $H^x$ und $H^y$, abhängig von den Variablen $\eta$ und $\xi$, nach $x$ und $y$ zu berechnen, wird die Kettenregel genutzt.

$$ \frac{\partial H^x(\xi,\eta)}{\partial \xi} = \frac{\partial H^x(\xi,\eta)}{\partial x} \cdot \frac{\partial x}{\partial \xi} + \frac{\partial H^x(\xi,\eta)}{\partial y} \cdot \frac{\partial y}{\partial \xi}$$
$$ \frac{\partial H^x(\xi,\eta)}{\partial \eta} = \frac{\partial H^x(\xi,\eta)}{\partial x} \cdot \frac{\partial x}{\partial \eta} + \frac{\partial H^x(\xi,\eta)}{\partial \eta} \cdot \frac{\partial y}{\partial \eta}  $$

oder:

$$ 
\left[ \begin{array}{center} 
\frac{\partial H^x(\xi,\eta)}{\partial \xi} \\
\frac{\partial H^x(\xi,\eta)}{\partial \eta}
\end{array}\right]
= 
\left[ \begin{array}{center} 
...
\cdot
\left[ \begin{array}{center} 
\frac{\partial H^x(\xi,\eta)}{\partial x} \\
\frac{\partial H^x(\xi,\eta)}{\partial y}
\end{array}\right]
$$

Somit kann mit Hilfe der inversen Jacobi-Matrix $\mathbf{J^{-1}}$ die Ableitung der Funktionen $H^x$ und $H^y$ wie folgt ausgedrückt werden:
...
$$
\begin{align}
F_x(\xi,\eta) &= x(\xi,\eta) = \sum_{i=1}^8 N_i(\xi,\eta) \cdot x_i^e \\
F_y(\xi,\eta) &= y(\xi,\eta) = \sum_{i=1}^8 N_i(\xi,\eta) \cdot y_i^e
\end{align}
$$
mit 
$$
...
x^e =
\left[ \begin{array}{r}  
x_1^e & x_2^e & x_3^e & x_4^e & x_5^e & x_6^e & x_7^e & x_8^e
\end{array}\right]^T \\
y^e =
\left[ \begin{array}{r} 
y_1^e & y_2^e & y_3^e & y_4^e & y_5^e & y_6^e & y_7^e & y_8^e
\end{array}\right]^T
\end{align}
$$

Eingesetzt in die Jacobi-Matrix ergibt sich 
$$
\begin{align}
\mathbf{J} &=
\left[ \begin{array}{center} 
\sum_{i=1}^8 \frac{ N_i(\xi,\eta)}{\partial \xi} \cdot x_i^e & \sum_{i=1}^8 \frac{N_i(\xi,\eta)}{\partial \xi} \cdot y_i^e \\
\sum_{i=1}^8\frac{ N_i(\xi,\eta)}{\partial \eta} \cdot x_i^e & \sum_{i=1}^8 \frac{N_i(\xi,\eta)}{\partial \eta} \cdot y_i^e
\end{array}\right] \\
&=
\left[ \begin{array}{center} 
\frac{N_1}{\partial \xi}   & \frac{N_2}{\partial \xi}   & \frac{N_3}{\partial \xi}   & \frac{N_4}{\partial \xi}&
\frac{N_5}{\partial \xi}   & \frac{N_6}{\partial \xi}   & \frac{N_7}{\partial \xi}   & \frac{N_8}{\partial \xi} \\
\frac{N_1}{\partial \eta}  & \frac{N_2}{\partial \eta}  & \frac{N_3}{\partial \eta}  & \frac{N_4}{\partial \eta}&
\frac{N_5}{\partial \eta}  & \frac{N_6}{\partial \eta}  & \frac{N_7}{\partial \eta}  & \frac{N_8}{\partial \eta}
\end{array}\right]
\cdot
\left[ \begin{array}{center} 
x_1^e & y_1^e \\
...
TODO: Herleitung hierhin...
$$ \frac{H^x(\xi,\eta)}{\partial x} = j_{11} \cdot \frac{H^x}{\partial \xi} + j_{12} \cdot \frac{H^x}{\partial \eta} $$
$$ \frac{H^y(\xi,\eta)}{\partial y} = j_{21} \cdot \frac{H^y}{\partial \xi} + j_{22} \cdot \frac{H^y}{\partial \eta} $$
...
_Frage zur Erstellung der Jacobi-Matrix_

Wenn ich es richtig verstanden habe wird $F$, also die Transformation von den physikalischen zu den Referenzkoordinaten nach der folgenden Formel berechnet

bei 1D Problemen (z.B. Balken)

$$
\begin{align}
F_e(\xi) = x(\xi) &= \sum_{i=1}^n N_i^e(\xi) \cdot x_i^e \\
&= \frac{1-\xi}{2} \cdot x_1^e + \frac{1+\xi}{2} \cdot x_2^e \\
&= \frac{1}{2} \cdot x_1^e - \frac{\xi}{2} \cdot x_1^e +\frac{1}{2} \cdot x_2^e + \frac{\xi}{2} \cdot x_2^e 
\end{align}
$$
...
bei 2D Problemen (z.B. Platten)

$$
\begin{align}
F_e(\xi,\eta) = x(\xi,\eta) = \sum_{i=1}^n N_i^e(\xi,\eta) \cdot x_i^e \\
F_e(\xi,\eta) = y(\xi,\eta) = \sum_{i=1}^n N_i^e(\xi,\eta) \cdot y_i^e
\end{align}
$$

Bei dem Paper von Batoz und Tahar wird als Berechnungsbasis das 8-Knoten Serendipity Element genutzt. Somit gibt es acht Basisfunktionen, acht x-Koordinaten und acht y-Koordinaten. Es ergibt sich daraus die folgende Jacobi-Matrix:
